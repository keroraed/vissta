...
public sealed record CreateCategoryCommand(string Name, string Slug, int? ParentCategoryId) : IRequest<int>;
public sealed record UpdateCategoryCommand(int Id, string Name, string Slug, int? ParentCategoryId) : IRequest<bool>;
    ...
    IRequestHandler<DeleteCategoryCommand, bool>
{
    public Task<IReadOnlyCollection<CategoryDto>> Handle(GetCategoryListQuery request, CancellationToken cancellationToken)
    {
        var items = categories.QueryReadOnly()
            .OrderBy(x => x.Name)
            .Select(x => new CategoryDto(x.Id, x.Name, x.Slug, x.ParentCategoryId))
            .ToList();
    ...
    public async Task<CategoryDto?> Handle(GetCategoryByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        return category is null ? null : new CategoryDto(category.Id, category.Name, category.Slug, category.ParentCategoryId);
    }
